...
void Camera::SetProjectionValues(float fovDegrees, float aspectRatio, float nearZ, float farZ) {
	float fov_radians = (fovDegrees / 360.0f) * XM_2PI;
	this->projectionMatrix = XMMatrixPerspectiveFovLH(fov_radians, aspectRatio, nearZ, farZ);
...
void Camera::SetRotation(const XMVECTOR& rot) {
	this->rotVector = rot;
	XMStoreFloat3(&this->pos, this->rotVector);
	UpdateViewMatrix();
}
...
void Camera::UpdateViewMatrix() {
	...
	//матрица вида (для преброобразования вершин из мировой системы координат в систему координат камеры)
